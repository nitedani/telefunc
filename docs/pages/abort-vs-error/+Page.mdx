...
Whereas `throw new Error('[Wrong getUser() usage] ...')` shoud *never* occur: if you make a typo and call `getUser({ permission: 'admni' })` then it's an internal bug in your backend code that should be fixed.
