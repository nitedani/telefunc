...
type RequestContext = {
  ...
  /** Register a callback fired once the telefunction body has settled (returned, thrown,
   *  or aborted) — before extension hooks and serialization. Fires exactly once. */
  onTelefunctionSettled: (cb: () => void) => void
  /** @internal Called by executeTelefunction right after the telefunction settles. */
  markTelefunctionSettled: () => void
...
function createRequestContext(request: Request): RequestContext {
  ...
  const fireSettled = () => {
    if (settled) return
    settled = true
    for (const cb of settledCallbacks) {
      try {
        cb()
      } catch {
        // User callback errors are silently swallowed
  ...
  const ctx: RequestContext = {
    ...
    markTelefunctionSettled: fireSettled,
    onClose(cb) {
      if (closed) {
        cb()
        return
